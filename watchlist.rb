...
def watchlist_entries
  watchlist = []
  watchlist += streetcomplete_query
...
def streetcomplete_query
  watchlist = []
  query = '/*
This has been generated by the overpass-turbo wizard.
The original search was:
“highway=bus_stop and wheelchair=no”
*/
[out:json][timeout:2500];
// gather results
(
  // query part for: “highway=bus_stop and wheelchair=no”
  node["highway"="bus_stop"]["wheelchair"="no"];
  way["highway"="bus_stop"]["wheelchair"="no"];
  relation["highway"="bus_stop"]["wheelchair"="no"];
);
// print results
out body;
>;
out skel qt;'
  ...
  info_message = '["highway"="bus_stop"]["wheelchair"="no"]'
  watchlist << { list: get_list_from_arbitrary_query(query).sample(20), message: info_message }
  return watchlist
end
