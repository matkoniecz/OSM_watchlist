...
def is_present_in_list_of_hashs(list, key_searched, value_searched)
  ...
  list.each{|tags|
    key = tags.keys[0]
    value = tags[key]
    if key == key_searched and value == value_searched
      not_found = false
      break
    end
  }
...
def search_for_missing_areas(branch, areas, zlevel, on_water)
  ...
  last_key = ''
  list_of_documented.each {|tag_info|
    if CartoCSSHelper::Info.rendered_on_zlevel({tag_info.key => tag_info.value}, 'area', zlevel, on_water)
      ...
      if not_found
        ...
        if area.is_output_different(node)
...
def get_dominant_color(filename, neutral_color=nil)
  ...
  (0..133).each{|delta|
    ...
    if neutral_color != nil
      if pixel == neutral_color
        next
      end
    end
    if colors[pixel] == nil
...
def get_dominant_colors(branch, tags, zlevel, on_water, type, folder=nil)
  ...
  tags.each{|tag|
    ...
    name = ''
    (0..(tag.keys.length-1)).each{|i|
      key = tag.keys[i]
      value = tag[key]
      if key == 'area' or key == 'name'
        next
      end
      if name != ''
        name += ' '
...
def compute_neighbours_to(branch, color_as_pixel, name, list_of_tags_and_dominant_color)
  CartoCSSHelper::Git.checkout(branch)
  list_of_tags_and_dominant_color.each{|i|
...
def compute_all_deltas(list_of_tags_and_dominant_color)
  returned = []
  list_of_tags_and_dominant_color.each{|i|
    list_of_tags_and_dominant_color.each{|t|
...
def compute_closest_among_all(list_of_tags_and_dominant_color)
  list_of_tags_and_dominant_color.each{|i|
    min_real_delta = 10000000
    closest = nil
    list_of_tags_and_dominant_color.each{|t|
      lab_a = i[:color].to_lab
      lab_b = t[:color].to_lab
      delta = i[:color].delta_e94(lab_a, lab_b)
      if delta > 0 and delta < min_real_delta
...
def generate_log_about_deltas(list_of_tags_and_dominant_color, count, assume_neighbour_to_be_known=false)
  ...
  list_of_tags_and_dominant_color.each_with_index {|e, i|
...
def generate_images_about_neighbours(branch, areas, zlevel)
  ...
  list_of_tags_and_dominant_color.each { |e|
...
def check_color(hex, name)
  hex = hex.gsub('#', '').scan(/../).map {|color| color.to_i(16)}
...
def edelta_run #'older_master_but_after_last_landcover_change'
  ...
  ['titan_blue', 'imagico_renewed', 'ren2', 'imagico_blue', 're2re', 'red'].each {|branch|
    [16].each {|zlevel| #10, 13, 20
